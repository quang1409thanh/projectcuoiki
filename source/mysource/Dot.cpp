...
void Dot::move(Brick brick[],int n,Paddle paddle,int &count_Broken_Bricks,int &COUNT_DIES,bool & sfx,Mix_Chunk *ballcollision,Mix_Chunk *brickcollision ){
...
void Dot::move5(Brick brick[],int n,Paddle paddle,int &count_Broken_Bricks,int &COUNT_DIES,bool & sfx,Mix_Chunk *ballcollision,Mix_Chunk *brickcollision ){
    // dành cho trường hợp là dot đang ở trên paddle
    if(mVelX==0&&mVelY==0){
        mPosX=paddle.getPosXP()+paddle.PADDLE_WIDTH/2-DOT_WIDTH/2;
        mPosY=SCREEN_HEIGHT-SCREEN_BOTTOM- DOT_HEIGHT-paddle.PADDLE_HEIGHT;
...
void Dot::render(const int& color,SDL_Renderer*& gRenderer)
{
    switch(color){
        case red:
...
void Dot::set_gDot_Texture(std::string path,const int &n,SDL_Renderer*& gRenderer) {
